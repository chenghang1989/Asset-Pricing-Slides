...
\begin{frame}{Face 1 --- Risk Premium as $-\Cov(R,\widetilde m)$ (Step 2)}
\small
Start from the raw SDF $1=\E_t[mR^i]$. The risk-free gross return fixes its scale:
\[
1=\E_t(m)R_f \quad\Longrightarrow\quad \E_t(m)=\tfrac{1}{R_f}.
\]
It is cleaner to carry the \textbf{normalized SDF}
\[
\widetilde m \equiv \frac{m}{\E_t(m)}=R_f m,\qquad \E_t(\widetilde m)=1.
\]
For the gross excess return $R^{i,e}\equiv R^i-R_f$, zero cost gives
\begin{align*}
0 &= \E_t(m)\,\E_t(R^{i,e})+\Cov_t(m,R^{i,e}),\\
\E_t(R^{i,e})
&= -\frac{\Cov_t(m,R^{i,e})}{\E_t(m)}
=-R_f\Cov_t(m,R^{i,e})
=-\Cov_t(\widetilde m,R^{i,e}).
\end{align*}
\[
\boxed{\ \E_t(R^i)-R_f=-\Cov_t(\widetilde m,R^i)\ }.
\]
Lognormal case, with $s\equiv\log\widetilde m$: $\ \E_t[r_i-r_f]+\sigma_i^2/2=-\Cov_t(s,r_i).$
\begin{block}{The conceptual heart}
Only \alert{covariance with the SDF} is priced: payoff in high-$\widetilde m$ states is \textbf{insurance} $\Rightarrow$ low premium; idiosyncratic risk uncorrelated with $\widetilde m$ earns \textbf{nothing}.
\end{block}
\end{frame}

\begin{frame}{Face 2 --- Beta Pricing: the SDF \emph{is} a Factor Model (Step 3)}
\small
Project $\widetilde m$ onto the space of payoffs $X$: $\ \widetilde m = x^{*} + \varepsilon$ with $x^{*}=\mathrm{proj}(\widetilde m\mid X)$ and $\E(\varepsilon\,X)=0$. Since $\varepsilon$ is orthogonal to all excess returns, $x^{*}$ prices all risk premia. Rewriting Face 1:
\begin{align*}
\E_t(R^i)-R_f &= -\,\Cov_t(\widetilde m,R^i)\\
&= \underbrace{\frac{\Cov_t(R^i,\widetilde m)}{\Var_t(\widetilde m)}}_{\textstyle \beta_{i,\widetilde m}}
\cdot\underbrace{\Big(-\Var_t(\widetilde m)\Big)}_{\textstyle \lambda_{\widetilde m}}\\
&= \beta_{i,\widetilde m}\,\lambda_{\widetilde m}.
\end{align*}
If $\widetilde m$ is \textbf{linear in factors}, $\widetilde m=a+b'f$, the same algebra gives the multifactor form
\[
\boxed{\ \E_t(R^i)-R_f=\beta_i'\,\lambda\ },\qquad \beta_i=\Var(f)^{-1}\Cov(f,R^i).
\]
\textbf{CAPM} is the special case $\widetilde m=a+b\,R^{W}$ (quadratic utility / market efficiency).
\begin{block}{Punchline (revisited in Frontier I)}
$1=\E(mR)$ and expected-return--beta pricing are \alert{mathematically equivalent representations of the same object}: ``estimate the SDF'' $=$ ``find the factors.''
\end{block}
\end{frame}

\begin{frame}{Face 3 --- MV Frontier \& the Hansen--Jagannathan Bound (Step 4)}
\small
From Face 1, for any gross excess return $R^{e}$:
\begin{align*}
\E(R^{e})&=-\Cov(\widetilde m,R^{e})
=-\rho_{\widetilde m,R^e}\sigma(\widetilde m)\sigma(R^{e}),\\
\frac{\big|\E(R^{e})\big|}{\sigma(R^{e})}
&= \big|\rho_{\widetilde m,R^e}\big|\,\sigma(\widetilde m)
\le \sigma(\widetilde m)= \frac{\sigma(m)}{\E(m)} .
\end{align*}
Maximizing the left side over all assets gives the \textbf{Hansen--Jagannathan bound}:
\[
\boxed{\ \sigma(\widetilde m)=\frac{\sigma(m)}{\E(m)}\ \ge\ \max_{R^e}\ \frac{\big|\E(R^{e})\big|}{\sigma(R^{e})}=\max\ \text{Sharpe ratio}\ }.
\]
The bound is \alert{tight} ($\rho_{\widetilde m,R^e}=-1$) at the \textbf{tangency / MV-efficient} portfolio: the normalized SDF is then spanned by the maximum-Sharpe portfolio.
\begin{block}{Two puzzles this seeds}
\begin{itemize}\itemsep1pt
\item \textbf{Equity-premium puzzle:} a $\sim$0.5 market Sharpe requires a volatile SDF (\href{https://doi.org/10.1016/0304-3932(85)90061-3}{\textcolor{blue}{Mehra--Prescott (1985)}}).
\item \textbf{Factor-war criterion (L07):} better factors attain a higher max squared Sharpe ratio (\href{https://doi.org/10.1016/j.jfineco.2018.02.012}{\textcolor{blue}{Fama--French (2018)}}).
\end{itemize}
\end{block}
\end{frame}
...
\begin{frame}{Why the News Split Matters}
\footnotesize
\RaggedRight
The decomposition turns a realized return into a diagnostic question: \alert{which expectation moved?}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.49\textwidth}
\textbf{\textcolor{blue}{Cash-flow news: fundamentals}}
\begin{itemize}\scriptsize\itemsep1pt
  \item Revisions to future dividends, earnings, productivity, or payout.
  \item Raises prices by increasing the valuation numerator.
  \item More visible in firm-level data, where idiosyncratic fundamentals move a lot.
\end{itemize}
\end{column}
\begin{column}{0.49\textwidth}
\textbf{\textcolor{blue}{Discount-rate news: required returns}}
\begin{itemize}\scriptsize\itemsep1pt
  \item Revisions to future expected returns / risk premia.
  \item Good $N_{DR}$ for future investors is bad for today's price: prices fall now, expected returns rise later.
  \item Persistent small changes in discount rates have large present-value effects.
\end{itemize}
\end{column}
\end{columns}
...
\begin{itemize}\scriptsize\itemsep2pt
  \item \textbf{Time series:} predictability is discount-rate variation before the shock; $N_{DR}$ is the same object after the shock.
  \item \textbf{Cross-section:} priced factors are exposures to bad state-variable news, not arbitrary covariances.
  \item \textbf{Empirical route:} estimate a VAR state vector and map shocks into revisions of expected cash flows and returns (\href{https://doi.org/10.2307/2233809}{\textcolor{blue}{Campbell 1991}}; \href{https://doi.org/10.1111/j.1540-6261.1993.tb04700.x}{\textcolor{blue}{Campbell--Ammer 1993}}).
\end{itemize}
\end{frame}
