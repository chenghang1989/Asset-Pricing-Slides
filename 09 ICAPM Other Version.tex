...
% ============================================================================
% Roadmap: Lectures 08-09 form one continuous derivation
% ============================================================================
\begin{frame}{Roadmap: Lecture 09 in Context}
	\begin{center}
		\small
		\begin{tikzpicture}[node distance=8pt,
			...
			\node[stage] (a) {Power utility};
			\node[stage, right=of a] (b) {Epstein-Zin preferences \\ (08 \S1)};
			\node[stage, below=12pt of b] (c) {Primitive SDF in $V_{t+1}/\mu_t$ \\ (08 \S2)};
			\node[here, below=12pt of c] (d) {\textbf{Wealth-return SDF} \\ \textbf{(here, 09 \S1)}};
			\node[here, right=of d] (e) {\textbf{Consumption-free SDF} \\ \textbf{(here, 09 \S2)}};
			\node[here, below=12pt of d] (f) {\textbf{Merton two-beta ICAPM} \\ \textbf{+ empirical applications (09 \S3)}};
			\draw[->,thick] (a) -- (b);
			\draw[->,thick] (b) -- (c);
			\draw[->,thick] (c) -- (d);
			\draw[->,thick] (d) -- (e);
	...
	\end{center}
	\vspace{4pt}
	\begin{block}{What this lecture does}
		Take the SDF from 08 and \textbf{operationalize it twice}: first in terms of the wealth return $R_{W,t+1}$ (Section 1), then in terms of \emph{news about future returns} (Section 2). The result is the Merton-Campbell ICAPM, which we then take to data (Section 3).
	\end{block}
\end{frame}

% ============================================================================
% Recap: what we inherit from 08
% ============================================================================
\begin{frame}[shrink=8]{Recap: The Primitive SDF and Why It Is Not Usable}
	\textbf{From Lecture 08:}
	\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=Primitive Epstein-Zin SDF]
	\begin{equation*}
		M_{t+1} = \delta \left( \frac{C_{t+1}}{C_t} \right)^{-1/\psi} \left( \frac{V_{t+1}}{\mu_t(V_{t+1})} \right)^{-(\gamma-1/\psi)}
	\end{equation*}
	\end{tcolorbox}
	\textbf{Empirical obstacles:}
	\begin{enumerate}
		\item $V_{t+1}$ is the entire continuation utility -- not in any dataset.
		\item Even $C_{t+1}$ in macro data is poorly measured (low frequency, smoothing, stockholder heterogeneity).
	\end{enumerate}
	\vspace{2pt}
	\begin{block}{Plan of this lecture}
		\begin{itemize}
			\item \S 1 -- Replace $V_{t+1}/\mu_t(V_{t+1})$ by an expression involving the wealth return.
			\item \S 2 -- Use Campbell's log-linearization of the budget constraint to also eliminate $C_{t+1}$, leaving only $r_{w,t+1}$ and \textit{news} about future $r_{w}$.
			\item \S 3 -- Read the result as a Merton two-beta ICAPM and review the empirical literature.
		\end{itemize}
	\end{block}
\end{frame}

% ============================================================================
% Section: Step 1 -- Wealth-Return Form (renamed from "Wealth Version")
% ============================================================================
\section{Step 1: Wealth-Return Form}
...
\begin{frame}[allowframebreaks]
    ...
    \begin{block}{Notation warning}
        $\psi_t$ with a time subscript is the \textbf{consumption-wealth ratio}. The preference parameter $\psi$ without a subscript is the \textbf{elasticity of intertemporal substitution}. They are different objects.
    \end{block}
...
\begin{frame}[allowframebreaks]
    \frametitle{Step 4a: Substitute the Conjecture into the Baseline SDF}

    \textbf{Baseline SDF (from Lecture 08):}
    \begin{equation*}
        M_{t+1} = \delta \left( \frac{C_{t+1}}{C_t} \right)^{-1/\psi} \left( \frac{V_{t+1}}{\mu_t(V_{t+1})} \right)^{-(\gamma-1/\psi)}
    \end{equation*}

    \textbf{From Steps 1--2 we have two substitutions:}
    \begin{itemize}
    	\item Value-function conjecture: $V_{t+1} = \phi_{t+1}\,W_{t+1}$.
    	\item Certainty equivalent: $\mu_t(V_{t+1}) = (W_t - C_t)\,\mu_t^*$, where $\mu_t^*\equiv\bigl(E_t[\phi_{t+1}^{1-\gamma}(1+R_{W,t+1})^{1-\gamma}]\bigr)^{1/(1-\gamma)}$.
    \end{itemize}

    \textbf{Plug in:}
    \begin{equation*}
        \frac{V_{t+1}}{\mu_t(V_{t+1})} = \frac{\phi_{t+1}\,W_{t+1}}{(W_t - C_t)\,\mu_t^*}.
    \end{equation*}
    ...
    \textbf{Use the budget constraint $W_{t+1} = (W_t - C_t)(1 + R_{W,t+1})$:}
    \begin{equation*}
    	\frac{V_{t+1}}{\mu_t(V_{t+1})} = \frac{\phi_{t+1}\,(1 + R_{W,t+1})}{\mu_t^*}.
    \end{equation*}

    \textbf{Therefore the SDF becomes:}
    \begin{equation}\label{eq:M-after-sub}
    ...
    \begin{block}{What is left to do}
    	The unobservable $V_{t+1}$ has been replaced by the observable $1+R_{W,t+1}$, but we still have $\phi_{t+1}/\mu_t^*$. Step~4b uses the FOC to nail this ratio down.
    \end{block}
\end{frame}

\begin{frame}[allowframebreaks]
    \frametitle{Step 4b: Solving for $\phi_t$ from the FOC}

    \textbf{Setup.} Recall (Step 2) that with $\psi_t \equiv C_t/W_t$ and $\rho \equiv 1-1/\psi$,
    \begin{equation*}
    	\phi_t^\rho = (1-\delta)\psi_t^\rho + \delta(1-\psi_t)^\rho (\mu_t^*)^\rho.
    \end{equation*}
    \textbf{FOC for consumption (Step 3):}
    \begin{equation}\label{eq:FOC-step3}
    	(1-\delta)\psi_t^{\rho-1} = \delta(1-\psi_t)^{\rho-1}(\mu_t^*)^\rho.
    \end{equation}

    \framebreak

    \textbf{Trick.} Multiply both sides of \eqref{eq:FOC-step3} by $(1-\psi_t)$:
    \begin{equation*}
    	(1-\delta)\psi_t^{\rho-1}(1-\psi_t) = \delta(1-\psi_t)^{\rho}(\mu_t^*)^\rho.
    \end{equation*}
    Plug into the formula for $\phi_t^\rho$:
    \begin{equation*}
    	\begin{aligned}
    		\phi_t^\rho &= (1-\delta)\psi_t^\rho + (1-\delta)\psi_t^{\rho-1}(1-\psi_t) \\
    		&= (1-\delta)\psi_t^{\rho-1}\bigl[\psi_t + (1-\psi_t)\bigr] = (1-\delta)\psi_t^{\rho-1}.
    	\end{aligned}
    \end{equation*}
    ...
    \textbf{Key result:}
    \begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=Closed Form for $\phi_t$]
    \begin{equation}\label{eq:phi-closed}
    	\phi_t = (1-\delta)^{1/\rho}\,\psi_t^{(\rho-1)/\rho}.
    \end{equation}
    \end{tcolorbox}
    \textbf{And from \eqref{eq:FOC-step3}:}
    \begin{equation}\label{eq:mustar-closed}
    	(\mu_t^*)^\rho = \frac{1-\delta}{\delta}\left(\frac{\psi_t}{1-\psi_t}\right)^{\rho-1}.
    \end{equation}
    \textbf{Reading.} The agent's value scales with current wealth via a function of the consumption-wealth ratio. This is the workhorse identity behind everything that follows.
\end{frame}

\begin{frame}[allowframebreaks]
    \frametitle{Step 4c: Collecting Powers $\Rightarrow$ Wealth-Return SDF}

    \textbf{Goal:} eliminate the remaining unobservable ratio $\phi_{t+1}/\mu_t^*$ in
    \begin{equation*}
        M_{t+1}
        = \delta\left(\frac{C_{t+1}}{C_t}\right)^{-1/\psi}
        \left(\frac{\phi_{t+1}}{\mu_t^*}\right)^{-(\gamma-1/\psi)}
        (1+R_{W,t+1})^{-(\gamma-1/\psi)}.
    \end{equation*}

    \textbf{The composite parameter.} Define
    \begin{equation*}
    	\theta \equiv \frac{1-\gamma}{1-1/\psi} = \frac{1-\gamma}{\rho}.
    \end{equation*}
    Direct algebra gives the identity
    \begin{equation}\label{eq:theta-identity}
    	\gamma - \tfrac{1}{\psi} = (1-\theta)\,\rho.
    \end{equation}
    [\textit{Proof:} $(1-\theta)\rho = \rho - \rho\,(1-\gamma)/\rho = \rho - (1-\gamma) = (1-1/\psi) - (1-\gamma) = \gamma - 1/\psi.$ \quad $\square$]

    \framebreak

    \textbf{Step A. Convert $\phi_{t+1}/\mu_t^*$ into growth rates.}

    Let
    \begin{equation*}
        G_{c,t+1}\equiv \frac{C_{t+1}}{C_t}, \qquad R^W_{t+1}\equiv 1+R_{W,t+1}.
    \end{equation*}
    Since $C_t=\psi_tW_t$ and $W_{t+1}=(1-\psi_t)W_tR^W_{t+1}$,
    \begin{equation*}
        G_{c,t+1}
        =\frac{\psi_{t+1}W_{t+1}}{\psi_tW_t}
        =\frac{\psi_{t+1}(1-\psi_t)}{\psi_t}R^W_{t+1}.
    \end{equation*}
    Hence
    \begin{equation}\label{eq:gc-rw-psi-ratio}
        \frac{\psi_{t+1}(1-\psi_t)}{\psi_t}
        =\frac{G_{c,t+1}}{R^W_{t+1}}.
    \end{equation}

    \framebreak

    \textbf{Step B. Use the FOC identities for $\phi$ and $\mu^*$.}

    From Step 4b:
    \begin{equation*}
        \phi_{t+1}^{\rho}=(1-\delta)\psi_{t+1}^{\rho-1},
        \qquad
        (\mu_t^*)^\rho=\frac{1-\delta}{\delta}\left(\frac{\psi_t}{1-\psi_t}\right)^{\rho-1}.
    \end{equation*}
    ...
    \begin{equation*}
        \begin{aligned}
        \left(\frac{\phi_{t+1}}{\mu_t^*}\right)^\rho
        &=\delta\,\psi_{t+1}^{\rho-1}\left(\frac{1-\psi_t}{\psi_t}\right)^{\rho-1}\\
        &=\delta\left(\frac{\psi_{t+1}(1-\psi_t)}{\psi_t}\right)^{\rho-1}\\
        &=\delta\left(\frac{G_{c,t+1}}{R^W_{t+1}}\right)^{\rho-1}.
        \end{aligned}
    \end{equation*}
    The last line uses \eqref{eq:gc-rw-psi-ratio}.

    \framebreak

    \textbf{Step C. Raise to the required power.}

    Since $(\gamma-1/\psi)/\rho=1-\theta$ and $\rho-1=-1/\psi$,
    \begin{equation*}
        \boxed{
        \delta\left(\frac{\phi_{t+1}}{\mu_t^*}\right)^{-(\gamma-1/\psi)}
        =
        \delta^\theta
        \left(\frac{G_{c,t+1}}{R^W_{t+1}}\right)^{(1-\theta)/\psi}
        }.
    \end{equation*}
    This step is pure algebra: the FOC converts the shadow value ratio into consumption growth and the wealth return.
    ...
    Now collect powers:
    \begin{equation*}
        -\frac{1}{\psi}+\frac{1-\theta}{\psi}
        =-\frac{\theta}{\psi},
        \qquad
        -\frac{1-\theta}{\psi}-(1-\theta)\rho
        =-(1-\theta)\left(\frac{1}{\psi}+\rho\right)
        =-(1-\theta).
    \end{equation*}
    The last equality uses $\rho=1-1/\psi$.

    \begin{tcolorbox}[colback=green!5!white, colframe=green!75!black, title=Result of Step 4]
    \begin{equation*}
    	M_{t+1}
        = \delta^{\theta}
        \left(\frac{C_{t+1}}{C_t}\right)^{-\theta/\psi}
        (1+R_{W,t+1})^{-(1-\theta)}.
    \end{equation*}
    \end{tcolorbox}
    \textbf{What changed?} The primitive SDF no longer contains $V_{t+1}$ or $\mu_t(V_{t+1})$. It depends only on consumption growth and the wealth return.
\end{frame}
...
\begin{frame}[allowframebreaks]
    ...
    \begin{itemize}
        \item Both terms are present; $\theta$ and $1-\theta$ are algebraic loadings, not necessarily portfolio weights between 0 and 1
        \item Nests both consumption CAPM and traditional CAPM
        \item When $\gamma \neq 1/\psi$:
        \begin{itemize}
            \item $1-\theta \neq 0$, so the wealth-return component enters the SDF
            \item Assets covarying with wealth return carry a distinct risk premium
            \item The sign and size of the loading depend on the joint values of $\gamma$ and $\psi$
        \end{itemize}
    \end{itemize}
 \end{frame}
...
\begin{frame}[shrink=8]
    ...
    Under joint lognormality, the risk-free rate is determined by the log SDF:
    \begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black]
    \begin{align*}
        r_{f,t}
        &= -E_t[m_{t+1}]-\frac{1}{2}\mathrm{Var}_t(m_{t+1})\\
        &= -\theta\log\delta
        +\frac{\theta}{\psi}E_t[\Delta c_{t+1}]
        +(1-\theta)E_t[r_{w,t+1}]
        -\frac{1}{2}\mathrm{Var}_t\!\left[
    ...
    \textbf{Key insight:} the slope on expected consumption growth is governed by the IES channel, not by CRRA's forced $\gamma=1/\psi$ restriction.
...
% ============================================================================
% Bridge: why the wealth-form is not the end of the story
% ============================================================================
\begin{frame}[shrink=8]{Bridge from \S 1 to \S 2: Why Go Further?}
	\textbf{We have:} the wealth-return SDF $M_{t+1} = \delta^\theta(C_{t+1}/C_t)^{-\theta/\psi}(1+R_{W,t+1})^{-(1-\theta)}$.
	\vspace{4pt}
	\textbf{Two reasons this is still not enough.}
	\begin{enumerate}
		\item \textbf{Consumption data are noisy.} Measurement error, smoothing, low frequency, stockholder/non-stockholder split. We would prefer a representation with $r_w$ alone.
		\item \textbf{We want the Merton (1973) interpretation.} The original ICAPM says: factors are \textit{innovations in state variables that forecast future investment opportunities}. The wealth form does not yet show this structure.
	\end{enumerate}
	\vspace{4pt}
	\begin{block}{Campbell's plan (1993)}
		\begin{itemize}
			\item Log-linearize the budget constraint around the steady state.
			\item Forward-solve for $c_t-w_t$, getting an expression involving expected future $r_w$ and $\Delta c$.
			\item Use the consumption Euler equation $E_t[\Delta c_{t+1}] = \mu_m + \psi E_t[r_{w,t+1}]$ to eliminate consumption growth from the SDF innovation.
			\item Land at $\widetilde{m}_{t+1} = -\gamma\,\widetilde{r}_{w,t+1} - (\gamma-1)\,\widetilde{h}_{t+1}$, which is exactly the Merton two-beta ICAPM.
		\end{itemize}
	\end{block}
\end{frame}

% ============================================================================
% Section 2: Step 2 — Campbell (1993) eliminates consumption
% ============================================================================
\section{Step 2: Campbell (1993) -- Eliminating Consumption}
...
\begin{frame}{Setup: Notation and Budget Constraint}
    \small
	\begin{itemize}
		\item $ W_t $: total wealth, including human capital, at the beginning of period $t$
		\item $ C_t $: consumption at time $t$
		\item $ R^W_{t+1} $: gross simple return on the wealth portfolio from $t$ to $t+1$
		\item $ r_{w,t+1}\equiv \log R^W_{t+1} $: log wealth return
		\item $x_t\equiv c_t-w_t=\log(C_t/W_t)$: log consumption-wealth ratio
	\end{itemize}
	The representative agent's dynamic budget constraint can then be written as
	\begin{align}
		W_{t+1}=R^W_{t+1}\left(W_{t}-C_{t}\right),\\
		\frac{W_{t+1}}{W_{t}}=R^W_{t+1}\left(1-\frac{C_{t}}{W_{t}}\right).
	\end{align}
	or in logs:
	\begin{align}
		\begin{aligned}
    ...
    \begin{block}{Purpose of this step}
        We are turning a nonlinear budget constraint into a nearly linear equation in $r_w$, $\Delta c$, and $c-w$ so that it can be forward-solved.
    \end{block}
\end{frame}

\begin{frame}{Linearizing the Budget Constraint}
    \small
	...
	The derivative is
    \begin{equation*}
        \frac{d}{dx}\log(1-\exp x)=-\frac{\exp x}{1-\exp x}.
    \end{equation*}
    Define
    \begin{equation*}
        \rho \equiv 1-\exp(\bar{x}).
    \end{equation*}
    Then
    \begin{equation*}
        -\frac{\exp(\bar{x})}{1-\exp(\bar{x})}
        =-\frac{1-\rho}{\rho}
        =1-\frac{1}{\rho}.
    \end{equation*}
	When the log consumption-wealth ratio is constant, $ \rho $ can be interpreted as $ (W-C)/W $, the constant ratio of invested wealth to total wealth.
\end{frame}

\begin{frame}{Difference Equation for the Consumption-Wealth Ratio}
    \small
	So the inter-temporal budget constraint is
	\begin{align}
		\Delta w_{t+1} \approx r_{w,t+1}+k+\left(1-\frac{1}{\rho}\right)\left(c_{t}-w_{t}\right)
	\end{align}
	use the accounting identity
	\begin{align}
		\Delta w_{t+1}= \Delta c_{t+1}+\left(c_{t}-w_{t}\right)  -\left(c_{t+1}-w_{t+1}\right)
	\end{align}
    This identity simply rewrites wealth growth using consumption growth and the change in the consumption-wealth ratio. Substituting it into the linearized budget constraint gives
	\begin{align}
    ...
    Equivalently,
    \begin{equation*}
        c_t-w_t=\rho\left[(c_{t+1}-w_{t+1})+r_{w,t+1}-\Delta c_{t+1}+k\right].
    \end{equation*}
\end{frame}

\begin{frame}{Forward-Solving the Difference Equation}
	\begin{align*}
...
\begin{frame}{The Consumption-Wealth Ratio Identity}
...
\begin{frame}{Decomposing the Consumption Innovation}
...
\begin{frame}[allowframebreaks]
    ...
    Apply the Euler equation to the wealth portfolio itself, $E_t[M_{t+1}(1+R_{W,t+1})]=1$. Then
...
\begin{aligned}
\widetilde{m}_{t+1}
&=-\frac{\theta}{\psi}\left[\widetilde{r}_{w,t+1}+(1-\psi)\widetilde{h}_{t+1}\right]
-(1-\theta)\widetilde{r}_{w,t+1}\\
&=-\left(\frac{\theta}{\psi}+1-\theta\right)\widetilde{r}_{w,t+1}
-\frac{\theta(1-\psi)}{\psi}\widetilde{h}_{t+1}.
\end{aligned}
\end{equation*}

Using $\theta=(1-\gamma)/(1-1/\psi)$,
\begin{equation*}
    \frac{\theta}{\psi}+1-\theta=\gamma,
    \qquad
    \frac{\theta(1-\psi)}{\psi}=\gamma-1.
\end{equation*}
...
% ============================================================================
% Section 3: Empirical applications of the ICAPM (renamed from "Rleadted Literature")
% ============================================================================
\section{Step 3: ICAPM Goes to Data}

\begin{frame}[shrink=8]{Reading the Two-Beta ICAPM as an Empirical Recipe}
	\textbf{From Section 2:}
	\begin{equation*}
		E_t[r_{i,t+1}] - r_{f,t+1} + \tfrac{\sigma_i^2}{2} = \gamma\,\sigma_{iw} + (\gamma-1)\,\sigma_{ih}.
	\end{equation*}
	\textbf{Empirical recipe.}
	\begin{enumerate}
		\item Pick a \textbf{state variable} $X_t$ that forecasts $r_{w,t+1}$ or $\sigma^2_{w,t+1}$.
		\item Use $\widetilde{X}_{t+1}$ as a proxy for the hedging factor news $\widetilde{h}_{t+1}$.
		\item Run a two-factor cross-sectional regression with $r_w$ and the chosen news factor.
		\item Check whether the implied $\gamma$ is economically sensible.
	\end{enumerate}
	\vspace{4pt}
	\begin{block}{Plan of \S 3}
		\begin{itemize}
			\item \S 3.1 Volatility-based ICAPM (Ang et al., Guo-Whitelaw, Herskovic et al.).
			\item \S 3.2 What counts as a valid state variable (Maio-Santa-Clara, Cooper-Maio).
			\item \S 3.3 The ``First Law of Finance'' debate over the risk-return tradeoff.
			\item \S 3.4 An empirical application to the Chinese stock market (Cheng-Guo-Shi, 2024).
		\end{itemize}
	\end{block}
\end{frame}

\subsection{ICAPM with Volatility State Variables}

\begin{frame}{Theoretical Motivation of \href{https://doi.org/10.1111/j.1540-6261.2006.00836.x}{\textcolor{blue}{Ang et al. (2006)}}}
	\begin{itemize}
		\item When investment opportunities vary over time, the multi-factor models of \href{https://doi.org/10.2307/1913811}{\textcolor{blue}{Merton (1973)}} and \href{https://doi.org/10.1016/0022-0531(76)90046-6}{\textcolor{blue}{Ross (1976)}} show that risk premia are associated with the conditional covariances between asset returns and innovations in state variables that describe the time-variation of the investment opportunities.
...
\subsection{What Counts as a Valid ICAPM State Variable?}
...
\subsection{The ``First Law of Finance'': Risk-Return Tradeoff}
...
\subsection{Empirical Application: China's Stock Market}
...
\begin{frame}{\href{https://www.sciencedirect.com/science/article/pii/S0378426624000372}{\textcolor{blue}{Cheng, Guo and Shi(2024)}}: Empirical Specifications}
		...
		$MV3_{t}=\omega_{0}+\beta^{\prime}\mathbf{Z}_{t-1}+\zeta_{t}$
	\\
	\vspace{.1in}
		$ERET_{t}=\phi+\gamma(\omega_{0}+\beta^{\prime}\mathbf{Z}_{t-1})+\lambda^{\prime}\mathbf{X}_{t-1}+\epsilon_{t}$
...
\begin{table}[ht]
	...
	\resizebox{110mm}{!}{
		\begin{tabular}{ccccccccccc}
			...
			\hline \\[-2ex]
				&\multicolumn{4}{c}{$MV3_{t}=\omega_{0}+\beta^{\prime}\mathbf{Z}_{t-1}+\zeta_{t}$}& \multicolumn{5}{c}{$ret_{t}=\phi+\gamma(\omega_{0}+\beta^{\prime}\mathbf{Z}_{t-1})+\lambda^{\prime}\mathbf{X}_{t-1}+\epsilon_{t}$}\\
...
% ============================================================================
% Closing summary: how 08-09 fit into the asset-pricing curriculum
% ============================================================================
\begin{frame}[shrink=20]{Closing the ICAPM Arc}
	\textbf{Where 08--09 sit in the bigger picture}:
	\vspace{4pt}
	\begin{center}
		\small
		\begin{tikzpicture}[node distance=6pt,
			box/.style={draw,rounded corners,align=center,inner sep=4pt,minimum height=20pt,font=\small}]
			\node[box,fill=gray!10] (a) {03 CAPM};
			\node[box,fill=gray!10,right=of a] (b) {05 FF Factor};
			\node[box,fill=gray!10,right=of b] (c) {06 Q-Factor};
			\node[box,fill=red!25,below=10pt of b] (d) {\textbf{08-09 ICAPM}};
			\node[box,fill=gray!10,right=of d] (e) {07 Factor Zoo};
			\draw[->,thick] (a) -- (b);
			\draw[->,thick] (b) -- (c);
			\draw[->,thick] (a) -- (d);
			\draw[->,thick] (d) -- (e);
	...
	\begin{block}{Three takeaways}
		\begin{enumerate}
			\item \textbf{The ICAPM is the theoretical bridge}: it tells us \emph{which} state variables can legitimately be priced factors -- those that forecast the distribution of future aggregate returns.
			\item \textbf{The Epstein-Zin engine} (08) plus Campbell's log-linearization (09) shows that the ICAPM emerges naturally once you separate risk aversion from intertemporal substitution.
			\item \textbf{Empirically} the volatility-based ICAPM and the wealth-and-hedging two-beta version unify many ``factor'' findings as priced state variables; this is the disciplined version of the empirical patches we saw in 05--07.
		\end{enumerate}
	\end{block}
	\vspace{2pt}
	\begin{center}
		\footnotesize \textit{08--09 are not a side track: they are the place where ``factor'' becomes a theoretical concept rather than a regression coefficient.}
	\end{center}
\end{frame}
