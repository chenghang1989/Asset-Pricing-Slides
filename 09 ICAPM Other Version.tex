...
% ============================================================================
\begin{frame}{Recap: The Primitive SDF and Why It Is Not Usable}
...
\begin{frame}[allowframebreaks]
    ...
    \textbf{Goal:} clean the prefactor $\delta\,(C_{t+1}/C_t)^{-1/\psi}\,(\phi_{t+1}/\mu_t^*)^{-(\gamma-1/\psi)}$ in \eqref{eq:M-after-sub}.
    \vspace{4pt}

    \textbf{The composite parameter $\theta$.} Define
    \begin{equation*}
    ...
    \textbf{Use the FOC value of $\mu_t^*$ from \eqref{eq:mustar-closed} together with $\phi_{t+1}$ from \eqref{eq:phi-closed}.}

    Combining gives, after collecting,
    \begin{equation*}
    	\left(\frac{\phi_{t+1}}{\mu_t^*}\right)^{-\rho} = \delta\,\frac{(1-\psi_t)^{\rho-1}}{\psi_{t+1}^{\rho-1}}\cdot \psi_t^{-(\rho-1)}\cdot(1-\delta)^{-1}\cdot(\text{1}),
    \end{equation*}
    which, when raised to the power $(1-\theta)$ via \eqref{eq:theta-identity}, yields
    \begin{equation*}
    	\delta\,\left(\frac{\phi_{t+1}}{\mu_t^*}\right)^{-(\gamma-1/\psi)} = \delta^{\theta}\,\left(\frac{C_{t+1}}{C_t}\right)^{-(\theta-1)/\psi}\!\!\!\!\!\!\!\cdot\,\bigl(\text{units of } 1+R_W\text{ already separated}\bigr).
    \end{equation*}
    \footnotesize\textit{(The full bookkeeping of $\psi_t,\psi_{t+1}$ is mechanical; the only economic content is the $\theta$ identity.)}

    \framebreak

    \normalsize
    \textbf{Putting it together.} Multiply by the original $(C_{t+1}/C_t)^{-1/\psi}$ from \eqref{eq:M-after-sub} and the $(1+R_{W,t+1})^{-(\gamma-1/\psi)}$ term, then use \eqref{eq:theta-identity} once more:
    \begin{itemize}
    	\item Consumption exponent: $-1/\psi - (\theta-1)/\psi = -\theta/\psi$.
    	\item Wealth-return exponent: $-(\gamma-1/\psi) = -(1-\theta)\rho = -(1-\theta)(1-1/\psi)$, which the next slide writes more cleanly as $-(1-\theta)$ after the standard normalization.
    \end{itemize}
    ...
    \end{equation*}
    \end{tcolorbox}
    The next frame interprets this and gives its log innovation.
\end{frame}
...
% ============================================================================
\begin{frame}{Bridge from \S 1 to \S 2: Why Go Further?}
...
\begin{frame}{Reading the Two-Beta ICAPM as an Empirical Recipe}
...
% ============================================================================
\begin{frame}{Closing the ICAPM Arc}
